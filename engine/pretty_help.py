...
from engine.settings import ANIMATEURID
...
class PrettyHelp(HelpCommand):
    ...
    def __init__(self, **options):
        ...
        self.commands_heading = options.pop("commands_heading", ":tada: Commands:")
    ...
    def add_indented_commands(self, commands, *, heading, max_size=None, group=None):
        ...
        for command in commands:
            if "help" in command.name:continue
            name = command.name
    ...
    def get_destination(self):
        ctx = self.context
        if not discord.utils.get(ctx.author.roles, id=ANIMATEURID):return
    ...
    async def send_bot_help(self, mapping):
        ...
        for command in filtered:
            ...
            if category not in added and category!=no_category:
